...
export function setupWebSocket(fastify) {
  ...
  fastify.addHook('onRequest', async (request, reply) => {
    if (request.url === '/ws-upload') {
      console.log(`[WS Handshake] Incoming request from ${request.ip}. Upgrade: ${request.headers.upgrade}`);
    }
  });
  ...
  fastify.get("/ws-upload", { websocket: true }, (connection, req) => {
    ...
    if (!ws) {
      console.error("❌ WS Error: No socket found in connection object");
      return;
    }
      ...
      try {
        if (!isBinary) {
          ...
          if (data.type === "init") {
            ...
            if (config.AUTH_TOKEN && token !== `Bearer ${config.AUTH_TOKEN}` && token !== config.AUTH_TOKEN) {
              console.log(`❌ WS Auth Failed: Invalid token`);
              ws.send(JSON.stringify({ type: "error", fileId: data.fileId, error: "Unauthorized" }));
            ...
            if (upload) {
              try {
                const finalFilePath = path.join(config.UPLOADS_DIR, `${Date.now()}-${upload.fileName}`);
                await mergeChunks(upload.chunkDir, finalFilePath, upload.totalChunks);
                ws.send(JSON.stringify({ type: "complete", fileId: data.fileId, success: true, url: `/uploads/${path.basename(finalFilePath)}` }));
              } catch (mergeError) {
                ws.send(JSON.stringify({ type: "complete", fileId: data.fileId, success: false, message: mergeError.message }));
              } finally {
