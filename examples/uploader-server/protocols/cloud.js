...
async function generateAzureSAS(blobName) {
  ...
  }, azureSharedKeyCredential).toString();
...
/**
 * Controller setup for Express
 */
export function setupCloud(app) {
  // S3 Presigned URL Endpoint
  app.post('/generate-s3-url', async (req, res) => {
    try {
      const { fileName, contentType } = req.body;
      if (!fileName) return res.status(400).json({ error: 'fileName is required' });
      
      const url = await generateS3PresignedUrl(fileName, contentType || 'application/octet-stream');
      res.json({ url });
    } catch (error) {
  ...
  // Azure SAS Endpoint
  app.post('/generate-azure-sas', async (req, res) => {
    try {
  ...
  app.post('/generate-gcs-url', async (req, res) => {
    try {
      const { fileName, contentType } = req.body;
      if (!fileName) return res.status(400).json({ error: 'fileName is required' });
      
      const url = await generateGCSSignedUrl(fileName, contentType || 'application/octet-stream');
      res.json({ url });
    } catch (error) {
