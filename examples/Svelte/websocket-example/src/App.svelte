<script>
  ...
  const uploader = useUploader({
    ...
    onBeforeRequest: async (file, chunk) => {
      console.log(
        `[Auth] Preparing WebSocket request for ${file.metadata.name}${chunk ? ` (Chunk ${chunk.index})` : ''}`,
      );
      return {
        headers: {
    ...
    onFilesRejected: (rejections) => {
      rejections.forEach((r) => {
        showNotification('File ' + r.file.file.name + ' rejected: ' + r.message, 'error');
      });
    },
    ...
    onAllComplete: (files) => {
      showNotification('All WebSocket uploads finished!', 'success');
    },
  });
...
<header>
  <h1>Svelte Uploader - WEBSOCKET example</h1>
  <p>
    This example demonstrates the <strong>streaming</strong> nature of the library in Svelte using WebSocket.
  </p>
</header>
