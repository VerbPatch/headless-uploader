...
export class MyUploader extends LitElement {
  @state() private authToken = 'verbpatch-secret-token';
  ...
  private uploader = useUploader(this, {
    ...
    onUploadError: (file: UploadFile, error: Error) => {
      this.showNotification('Error: ' + file.file.name + ' - ' + error.message, 'error');
    },
  ...
  render() {
    ...
    return html`
      <header>
        <h1>Lit Uploader - WEBTRANSPORT example</h1>
        <p>
          This example demonstrates the <strong>headless</strong> nature of the library in Lit using
          WebTransport (HTTP/3).
        </p>
      </header>
